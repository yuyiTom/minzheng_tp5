...
class AssetController extends AdminBaseController
{
    ...
    public function webuploader_self()
    {
        if ($this->request->isPost()) {

            $uploader = new Upload_self();
            ...
            if ($result === false) {
                ...
                $this->success("上传成功!_self", '', $result);
            }
            ...
            if (array_key_exists($arrData["filetype"], $arrFileTypes)) {
                ...
                // yuyi
                // 原来的是 10240 kb = 10M
                $fileTypeUploadMaxFileSize = '2048';
            } else {
                $this->error('上传文件类型配置错误！');
            }
            ...
            View::share('app', $arrData["app"]);
            // yuyi
            View::share('pname', $arrData["pname"]);

            // print_r(json_encode($arrData));exit;
            ...
            $this->assign('tab', $tab);
            return $this->fetch(":webuploader_self");
