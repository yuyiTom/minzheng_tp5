...
class Upload_self
{
    ...
    public function upload()
    {
        ...
        // return false;
// yuyi
       $fileSaveName    =  $pname. "." . $strFileExtension;
       $strSaveFilePath = './upload/mz/' . $fileSaveName; //TODO 测试 windows 下
        ...
        if ($objAsset && $storage['type'] =='Local') {
            ...
            if (file_exists('./upload/' . $arrInfo["file_path"])) {
                ...
                @rename($strSaveFilePath, './upload/mz/' . $fileSaveName);//yuyi
            }
...
// yuyi
        return [
            ...
            "name"        => $fileSaveName ,
            'id'          => $strId,
            'preview_url' => cmf_get_root() . '/upload/mz/' . $fileSaveName,
            'url'         => cmf_get_root() . '/upload/mz/' . $fileSaveName,
        ];
